...
`heartbeat_at` is refreshed by the per-session adapter process or by one-shot CLI snapshot commands. Stale session sweeps should treat a file as stale when the heartbeat is older than the configured threshold. The default stale heartbeat threshold is 5 minutes and should be configurable. PID is useful for diagnostics and lock recovery, but dead-PID sweeps must not immediately delete one-shot CLI snapshots.
...
|---|---|
| **session_start** | Sweep stale session files by heartbeat age. Register this session, including `agent_kind`, `adapter`, `repo_id`, branch, current integrated main SHA, and initial `dirty_files`. Compare the branch against the latest known local `main` and fetched `origin/main`; if it is already behind, seed `integration.pending` using the ancestry-based pending-target update rules. |
...
| `worktree-fleet land` | `/land` where slash commands exist | Safely lands the current non-integration worktree onto the local integration branch. It syncs first, requires clean current and integration worktrees, fast-forwards the integration branch, and emits the local main-advance event. |
...
- **Stale session files.** Session ends abnormally (crash, kill -9). The session file is left behind. The next `session_start` sweeps any session file whose heartbeat is stale. Normal session shutdown should remove the file immediately; one-shot CLI snapshots remain visible until their heartbeat expires.
...
- **Worktree removed externally.** A worktree directory is deleted while a session file still references it. The heartbeat sweep clears the stale snapshot after the configured threshold; in the meantime, fleet status may show a path that no longer exists. Acceptable; degrades gracefully.
